...
void CGameStateRun::OnMove()							// 移動遊戲元素
{
	// chest
	if (phase == 3) {
		if (character.Top() + character.Height() >= chest_and_key.Top() && character.Left() + character.Width() >= chest_and_key.Left() && character.Left() <= chest_and_key.Left() + chest_and_key.Width()) {
	...
	// door 
	if (phase == 5) {
		if (character.Top() <= door[0].Top() + door[0].Height() && character.Left() + character.Width() >= door[0].Left() && character.Left() <= door[0].Left() + door[0].Width()) {
			door[0].SelectShowBitmap(1);
		}
		if (character.Top() <= door[1].Top() + door[1].Height() && character.Left() + character.Width() >= door[1].Left() && character.Left() <= door[1].Left() + door[1].Width()) {
			door[1].SelectShowBitmap(1);
		}
		if (character.Top() <= door[2].Top() + door[2].Height() && character.Left() + character.Width() >= door[2].Left() && character.Left() <= door[2].Left() + door[2].Width()) {
...
// variable
int character_x = 150, character_y = 265;
...
void CGameStateRun::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	...
	// up
	if (nChar == VK_UP) { 
		character.SetTopLeft(character_x, character_y-=5);
	}
	...
	// left
	if (nChar == VK_LEFT) {
		character.SetTopLeft(character_x-=5, character_y);
	}
